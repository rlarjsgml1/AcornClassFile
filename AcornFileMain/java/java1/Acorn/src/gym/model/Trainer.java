...
import gym.interfaces.DietManageable;
...
public class Trainer extends Person implements Registerable, DietManageable {
	...
	public Trainer(String name, String phoneNumber, int age, double height, double weight) {
		...
		this.grade = TrainerGrade.INTERN; //grade를 기본으로 TrainerGrade에 INTERN로 초기값으로 잡음
	...
	public void manageDiet(Member member) {
		...
		if (bmi < 18.5) { //bmi 결과가 18.5 이하이면 출력
			System.out.println("저체중 → 고칼로리 및 단백질 식단 추천");
		} else if (bmi < 23.0) { //bmi 결과가 23.0 이하이면 출력
			System.out.println("정상 → 균형 잡힌 유지 식단 추천");
		} else if (bmi < 25.0) { //bmi 결과가 25 이하이면 출력
			System.out.println("과체중 → 저칼로리 및 고단백 식단 추천");
		} else { //bmi 결과가 이상이면 출력
	...
	private void updateGradeByMemberCount() {
		...
		if (count >= 10) { //회원이 10명이상이면 TrainerGrade에 MASTER을 등급으로 바뀜
			grade = TrainerGrade.MASTER; 
		} else if (count >= 7) { //회원이 7명이상이면 TrainerGrade에 SENIOR을 등급으로 바뀜
			grade = TrainerGrade.SENIOR;
		} else if (count >= 4) { //회원이 4명이상이면 TrainerGrade에 JUNIOR을 등급으로 바뀜
			grade = TrainerGrade.JUNIOR;
		} else { //회원 0~ 3까지는 INTERN로 표시
	...
	public void printInfo() {
		System.out.println("===" + getName() + "트레이너 정보" + "==="); //getName 해당 이름 가져와서 표시 
		System.out.println("이름: " + getName()); //해당 트레이너 이름 가져와서 보여주기
		System.out.println("전화번호: " + getPhoneNumber()); //해당 트레이너 전화번호 보여주기 
		System.out.println("나이: " + getAge()); //해당 트레이너 나이 보여주기
		System.out.println("키: " + getHeight()); //해당 트레이너 키 보여주기 
		System.out.println("몸무게: " + getWeight()); //해당 트레이너 몸무게 보여주기 
		System.out.println("등급: " + grade); //해당 트레이너 등급 보여주기 
		System.out.println("담당 회원 수: " + managedMembers.size()); //해당 담당회원수를 manageMembers.size로 해서 몇 명 보여주기
	}
