...
서버 작업은 아래 흐름으로 설명합니다.

```text
요청 -> Servlet/Controller -> Service -> DAO/DB -> 응답
```

화면 작업은 아래 흐름으로 설명합니다.
...
- 404: URL, context path, servlet mapping, JSP 위치 확인
- 500: Console 로그, 예외 메시지, DB 연결, null, SQL 확인
- DB 오류: SQL 에러 코드, 테이블명, 컬럼명, 바인딩 값 확인
